...
| `accessDurationDays` | number \| null | **Time-limited access (Checkpoint 3).** Only on `/api/courses/:id` (detail). Days of access a purchase grants (90\|180\|365); `null` = lifetime. Mutually exclusive with `purchaseMode === "sectional"`. Show it pre-purchase ("الوصول لمدة ٩٠ يومًا من تاريخ الشراء" / "وصول دائم") — web shows this at the consent moment and mobile should too. A purchase snapshots it onto the enrollment as `accessExpiresAt`; later edits to the course never change existing enrollments. |
...
| `accessExpiresAt` | string \| null | **Time-limited access (Checkpoint 3).** ISO timestamp after which the playback/study gate denies with `ACCESS_EXPIRED`. `null` = lifetime (every pre-feature enrollment). Always present (never omitted). Snapshotted at purchase from the course's `accessDurationDays`; a renewal purchase re-stamps the same enrollment. |
...
3. **Time-limited access (Checkpoint 3):** if `E.accessExpiresAt` is set AND
   `now > accessExpiresAt` → **the whole course is locked** (`ACCESS_EXPIRED`),
   regardless of everything below. Render an expired state with a renew
   affordance — renewal works in-app via `POST /api/enrollments`
   (wallet-funded, see the Writes table) or on the web. Web shows a lock
   screen with تجديد الوصول and the line
   "تقدمك محفوظ — كل بطاقاتك ونتائجك تبقى كما هي"; mirror that tone.
   `null`/absent stamp = lifetime; never treat it as expired.
...
6. If `V.sectionId` is missing/empty → unlocked (untagged video on a sectional
   course — the server grants and logs `sectional_untagged_video`).
7. If `V.sectionId ∈ E.ownedSectionIds` → unlocked.
8. Otherwise → locked.

This mirrors the server-side gate (`evaluateVideoAccess()` in
`lib/courses/videoAccess.ts`, wrapped by `/api/mux/playback-token`) exactly,
**including the check order** — the expiry check sits after the enrollment
gate and before the sectional branch, same as the server. The client only
renders affordances — the server is authoritative. **A client lock that does
not know about expiry lies to the student**: it would render a video as
playable and then fail at token time with `403 ACCESS_EXPIRED`. Any client
predicate must consume `accessExpiresAt` from `GET /api/me/enrollments`.
...
| POST | `/api/enrollments` | Buy a course with wallet | Body: `{ courseId }`. Server generates an idempotency key (`generateProtectionKey(uid, courseId)`) so duplicate retries from a flaky mobile network don't double-charge. Calls the same atomic Firestore transaction logic as `purchaseCourseWithWallet`: read wallet, check balance ≥ price (use `salePrice` if lower), block self-purchase, debit buyer, create `enrollments/{uid}_{courseId}`, log the buyer's `wallet_transactions` row, increment course `enrollmentCount`. **Instructor side (changed):** the sale no longer credits the instructor's spend wallet — it appends an immutable `earning` entry to `users/{instructorId}/earningsLedger` and increments `earningsTotal` (a real-world cash payable, see `docs/INSTRUCTOR_PAYOUTS.md`). This is a server-side change only; the request/response contract is unchanged. Free courses (`price === 0`) take a separate fast-path that mirrors `enrollInFreeCourse`. Returns `{ enrollmentId, newBalance }`. **Sectional courses (`course.purchaseMode === "sectional"`) are rejected with `COURSE_NOT_SECTIONAL` (400) before any wallet logic runs** — those must be purchased on the web via the sectional flow (Phase 7 plan is reader-app: no mobile sectional purchase). **Time-limited access (shipped 2026-07-10, Checkpoint 1):** courses may carry `accessDurationDays` (90\|180\|365; unset = lifetime); every enrollment-writing path snapshots `accessExpiresAt` (ISO string) onto the enrollment. The already-enrolled pre-check deliberately skips completed enrollments carrying `accessExpiresAt`, so a renewal re-runs the purchase actions and re-stamps the SAME enrollment doc (`computeAccessExpiresAt()` in `lib/courses/accessDuration.ts` — renewing early extends from current expiry). **Renewal from mobile (Checkpoint 3):** renewal IS available in-app through this same endpoint — same body, same response shape, wallet-funded only. Behavior matrix: lifetime enrollment (`accessExpiresAt: null`) → `409 ALREADY_ENROLLED` as before; time-limited enrollment (stamp set, expired or not) → the purchase proceeds as a renewal, charges the current course price, and returns the normal success shape. After a successful renewal, re-fetch `GET /api/me/enrollments` for the new `accessExpiresAt`. Renewal does not create a second enrollment and does not change `enrolledAt`. |
| POST | `/api/mux/playback-token` | Issue a signed Mux playback token | Body: `{ courseId, videoId }`. **DRM gate.** Steps as shipped: (1) verify Bearer token; (2) per-user rate limit, 30/min sliding window → `429 RATE_LIMITED` (fails open on Redis errors); (3) load course doc → `404 COURSE_NOT_FOUND` if missing, and also for non-privileged callers when `isCoursePubliclyVisible(course)` is false; (4) find the video by `videoId` → `404 VIDEO_NOT_FOUND`; `409 VIDEO_NOT_READY` if it has no `playbackId`; (5) determine access — owner (`createdBy === uid`), admin, free preview (`video.isFreePreview === true`), or completed enrollment at `enrollments/{uid}_{courseId}` → `403 NOT_ENROLLED` otherwise. **No free-course bypass — `price === 0` grants nothing without an enrollment.** **Time-limited access (shipped 2026-07-10, Checkpoint 1):** a completed enrollment whose `accessExpiresAt` stamp (ISO string, snapshotted at purchase on courses with `accessDurationDays`) has passed → `403 ACCESS_EXPIRED` — surface as "renew on web". Unset stamp = lifetime. Gate logic: `evaluateVideoAccess()` in `lib/courses/videoAccess.ts`. On sectional courses (`purchaseMode === "sectional"`), a sectional-scope enrollment must own the video's section → `403 SECTION_NOT_OWNED` (unset/`"full"` `accessScope` and untagged videos grant); (6) sign a Mux playback JWT for that `playbackId` with `aud: "v"`, empty custom claims, and `exp = now + 2 hours` (`TTL_SECONDS = 7200`). Return `{ playbackId, token, thumbnailToken, expiresAt }`. |

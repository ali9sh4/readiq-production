...
export class ZainCash {
  ...
  private generateToken(data: any): string {
    ...
    const payload = Buffer.from(JSON.stringify({ ...data, iat: Math.floor(Date.now() / 1000) })).toString("base64url");
  ...
  public verifyToken(token: string): any {
    ...
    const expectedSig = crypto.createHmac("sha256", this.secretKey).update(`${header}.${payload}`).digest("base64url");
    if (signature !== expectedSig) throw new Error("Invalid signature");
    return JSON.parse(Buffer.from(payload, "base64url").toString());
  }

  public async createTransaction(amount: number, orderId: string): Promise<any> {
    const token = this.generateToken({
      ...
      msisdn: this.msisdn,
      orderId,
      redirectUrl: `${process.env.NEXT_PUBLIC_APP_URL}/api/payment/zaincash/webhook`,
